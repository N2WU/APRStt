 ...
 * Module:      readaprstt.c
 *
 * Purpose:   	APRStt speech transmitter
 *		
 * Description:	For use in the West Point Triathlon to report DTMF commadns over voice.
 *
 ...
 */;
	...
	lon = secure_getenv ("TTLON");
	strcat(english1,call);
	strcat(english1,english2);
	strcat(english1,status);
	strcat(english1,english3);
	strcat(english1,lat);
	strcat(english1,english4);
	strcat(english1,lon);
 ...
 */
	      snprintf (reply_text, sizeof(reply_text), "N0CALL>SPEECH:%s", english1);
